...
const MessagePage = () => {
  ...
  useEffect(() => {
    AxiosInstance.get(`${process.env.REACT_APP_API_URL}/message/${idClient}/${idNurse}`)
      .then(({ data }) => {
        setHistory(data);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    if (user.rol === 0) {
      AxiosInstance.get(`${process.env.REACT_APP_API_URL}/nurse/nurse/${idNurse}`).then(
        ({ data }) => {
          setRecieved(data);
        }
      );
    ...
    if (user.rol === 1) {
      AxiosInstance.get(`${process.env.REACT_APP_API_URL}/client/${idClient}`).then(
        ({ data }) => {
          setRecieved(data);
        }
      );
  ...
  const handleSendMessage = () => {
    ...
    AxiosInstance.post(`${process.env.REACT_APP_API_URL}/message/${idClient}/${idNurse}`, {
      content: message,
      who: who,
    })
      .then(({ data }) => {
